...
def plot_growth(daily: pd.DataFrame) -> plt.Figure:
    """Two-panel growth-of-money chart: full history (log) and ALLW overlap window."""
    ...
    for ax, data, title, ylabel in [
        (axes[0], value, "Full available history", "Indexed value"),
        (axes[1], overlap, "ALLW overlap window", "Indexed value"),
    ]:
        style_ax(ax)
        for strategy in order_strategies(data.columns, STRATEGY_ORDER):
            ...
            if s.empty:
                continue
            ax.plot(s.index, s.values, color=COLORS.get(strategy), lw=2.0, label=strategy)
            ax.annotate(
                f"{s.iloc[-1]:.0f}",
                xy=(s.index[-1], s.iloc[-1]),
                xytext=(6, 0),
                textcoords="offset points",
                color=COLORS.get(strategy, TEXT_COL),
                fontsize=8,
                va="center",
            )
...
def plot_drawdowns(daily: pd.DataFrame) -> plt.Figure:
    ...
    for strategy in order_strategies(drawdowns.columns, STRATEGY_ORDER):
        ...
        if not s.empty:
            ax.plot(s.index, s.values, color=COLORS.get(strategy), lw=1, label=strategy)
...
def plot_calendar_profile(calendar: pd.DataFrame) -> plt.Figure:
    ...
    for ax, (metric, title) in zip(axes, metrics):
        ...
        for idx, strategy in enumerate(strategies):
            ...
            ax.bar(x + offset, values, width=width, color=COLORS.get(strategy), alpha=0.85, label=strategy)
        ...
        for strategy in order_strategies(rolling["Strategy"].unique(), STRATEGY_ORDER):
            for window, linestyle in [("1Y", "-"), ("3Y", "--")]:
                ...
                ax.plot(
                    ...
                    color=COLORS.get(strategy), linestyle=linestyle,
...
def plot_monthly_returns(monthly: pd.DataFrame) -> plt.Figure:
    ...
    for row, strategy in enumerate(strategies):
        ...
        ax_hist.hist(data["Return (%)"].dropna(), bins=24, color=COLORS.get(strategy), alpha=0.85)
...
def plot_risk_diagnostics(summary: pd.DataFrame) -> plt.Figure:
    ...
    for ax, metric in zip(axes.flat, metrics):
        ...
        ax.bar(x, vals, color=[COLORS.get(s) for s in strategies], alpha=0.85)
