...
DIY_FEE = 0.0012
ALLW_FEE = 0.0085
DEFAULT_OUTPUT_ROOT = Path(__file__).resolve().parent / "results" / "strategy_comparison"

DISPLAY_NAMES = {
    "DIY": "My Strategy (DIY)",
    "SPY": "S&P 500 (SPY)",
    "JEPQ": "JEPQ (JPM Nasdaq Income)",
...
def required_tickers(allocation: dict[str, float],
    ...
    tickers = set(allocation) | {"SPY", "ALLW", "JEPQ"}
    if include_6040:
        tickers.add("TLT")
...
def build_report_bundle(prices: pd.DataFrame,
    ...
    if "JEPQ" in prices:
        jepq_raw = prices["JEPQ"].dropna()
        if not jepq_raw.empty:
            values["JEPQ"] = jepq_raw / jepq_raw.iloc[0] * 100.0
...
def _manifest(strategy_id: str,
    ...
    return {
        ...
        "full_grid_top_leaderboard_row": {
            ...
            "source": "research/rsi_leverage_overlay/results/mixed_leverage_full_grid_oos/*/structural_full_grid_leaderboard.csv",
            "rank": 1,
            "spy_rule": "32/42 @ 30%",
            "gld_rule": "36/52 @ 30%",
            "global_cap": MIXED_CANDIDATE_GLOBAL_CAP,
