...
live/logs/performance_tracking_*.csv against the engine-simulated returns for
...
-----
  ...
  conda run -n allweather python -m research.backtest_shadow \
      --csv live/logs/performance_tracking_alpaca_paper_default.csv \
...
from research.allw_benchmark.compare_allw import _alloc_from_json, _strip_tz

_RESULTS_DIR = os.path.join(_SCRIPT_DIR, "results")
_LOGS_DIR    = os.path.join(_PROJECT_ROOT, "live", "logs")
...
def _find_performance_csv(logs_dir: str) -> list[str]:
    """Return all performance_tracking_*.csv files found in live/logs/."""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help=(
            ...
            "Omit to auto-detect from live/logs/ (first match used)."
        ),
    )
    p.add_argument(
