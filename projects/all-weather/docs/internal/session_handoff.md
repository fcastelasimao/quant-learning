...
**This session (2026-06-03) covered:** completed **Section D in full (D.12–D.19)** — US federal tax model (rate schedule + per-asset characterization incl. GLD 28% collectibles / GSG §1256 60/40), `LotLedger` with FIFO/HIFO/tax_optimal selectors, `RebalancePolicy` drift trigger in the engine (golden-locked, zero regression), tax-aware backtest engine, production-validation tax addendum, the D.18 threshold sweep (**verdict: drift beats monthly under US tax**), and three research docs. Next actionable: Section E (marimo) or the F.26 human gate.
...
| Item | State |
|---|---|
| Test suite | **331 passing**, 2 deselected, 1 deprecation warning (websockets, harmless) — full `-m "not integration"` run, 349s, exit 0. = 211 pre-D.12 baseline + 120 new across Section D (D.12 schedule 24, D.13 events 29, D.14 lot-ledger 24, D.15 golden-guard 4 + policy 16, D.16 tax-backtest 11, D.17 prod-validation-tax 5, D.18 sweep-verdict 7). Golden hash MATCH (`64c5753…`) confirms `engine/backtest.py` refactor is byte-identical on the default path. |
...
| D.19 | Docs: `docs/research/tax_model.md`, `docs/research/drift_trigger.md`, `docs/research/tax_threshold_sweep.md`. TQQQ/SQQQ style: what was tested, methodology, results, decision. | **DONE 2026-06-03** — all three written with real sweep numbers. |
...
| ID | What | Status |
|---|---|---|
| F.26 | Read D.18 verdict. If drift+tax wins: update `strategies.json` with `rebalance_policy` block; update `live/rebalance.py` to use drift trigger (currently 31-day cadence only); update `failed_strategies/weekly_rebalance/README.md` with new verdict; one paper-month of agreement before flipping live policy. | **READY — D.18 verdict PASSED.** Recommended candidate `drift_absolute(0.05)` (5pp, FIFO). **HUMAN gate before flipping live.** Before flip: (a) confirm sweep on yfinance total-return matches FMP (near-identical per CLAUDE.md 2026-05-09); (b) `strategies.json` `rebalance_policy` block; (c) `live/rebalance.py` drift gate alongside 31-day cadence; (d) reopen `failed_strategies/weekly_rebalance/README.md` verdict (tax-aware); (e) one paper-month agreement. **Do NOT auto-flip.** |
