...
│   ├── alpaca_rebalance.py   legacy Alpaca-only rebalancer (preserved for back-compat)
│   ├── rebalance.py          broker-agnostic rebalancer (--broker alpaca|tastytrade,
│   │                           --dry-execute, --budget, --min-rebalance-interval-days)
│   ├── budget.py             per-account virtual sub-portfolio cap (--budget AMOUNT)
...
│   ├── healthcheck.py        pre-flight check: env, creds, strategies, cadence
│   ├── brokers/              Broker protocol + concrete implementations
│   │   ├── base.py           Broker Protocol, PositionSnapshot, OrderResult, ActivityEvent
│   │   ├── factory.py        make_broker(broker_name, trading_mode, account_label)
│   │   ├── alpaca.py         AlpacaBroker (alpaca-py)
│   │   └── tastytrade.py     TastytradeBroker (community SDK, guarded import, qty-only)
│   └── scheduler/            launchd plist templates (macOS)
│
├── research/                 analyses that run periodically but are not production
│   ├── compare_allw.py       ALLW ETF head-to-head (now includes JEPQ benchmark)
...
- RSI ETF leverage overlay is research-only. Latest reviewed bundles: single-ETF `results/leverage_comparison/2026-05-11_12-15-40_6asset_tip_gsg_rpavg`; mixed SPY+GLD `results/mixed_leverage/2026-05-15_16-27-59_6asset_tip_gsg_rpavg`; mixed-pair OOS validation `results/mixed_leverage_oos_validation/2026-05-15_17-54-04_6asset_tip_gsg_rpavg` (all 4 fixed candidates and all 5 grid selectors pass 3/3 splits; `best_calmar` repeatedly picks SPY 34/42/0.25 + GLD 22/50/0.25 at 0.30 cap across IS windows).
...
- Mixed-pair OOS validation runner: `python -m research.validate_mixed_leverage_oos` or `make mixed-leverage-oos-validation`. It evaluates capped SPY+GLD MixedOverlayCandidate sets two ways: (a) fixed named candidates without re-selection, (b) coarse mixed grid (entry {22,26,30,34} × exit {42,50,58,66} × leverage {0.10,0.15,0.20,0.25} × cap {0.20,0.25,0.30} = 12,288 combos per split) with the same 5 selectors as single-ETF. Use `--fixed-only` for a fast smoke test.
- Next RSI overlay gate: add walk-forward/train-test validation on top of the mixed-pair OOS run before promoting any threshold to production.
...
- **Live execution** uses `live/rebalance.py` (broker-agnostic). The legacy `live/alpaca_rebalance.py` is kept unchanged for backward compatibility with tests. See `docs/BROKER_SETUP.md` for credential setup, budget cap, 31-day hold gate, notifications, and launchd scheduling.
