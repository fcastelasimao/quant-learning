...
def _repo_data_dir() -> str:
    ...
    Resolved by the shared ``quantcore`` engine (honours ``$QUANT_DATA_DIR``,
    then a walk-up search, then the known workspace fallback).
    """
    from quantcore import config as _qc_config
    return str(_qc_config.data_dir())
...
def fetch_dividends(tickers: list[str],
    ...
    Reads the `dividends` table (populated by `quantcore.ingest
