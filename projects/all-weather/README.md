...
```bash
git clone https://github.com/fcastelasimao/personal_projects.git
cd personal_projects/projects/all-weather
...
All commands must be run from the `all-weather/` project root. Use `conda run -n allweather` to stay inside the environment without activating it interactively.
